...
SELECT 
    ...
    'Citibank' AS "Bank",
     ...
     (SELECT COUNT(*) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_CITI WHERE TO_DATE(SPLIT_PART(TRANSACTION_POSTED_DATE, ' ', 1), 'DD/MM/YYYY') BETWEEN TO_DATE('01/04/2025', 'DD/MM/YYYY') AND CURRENT_DATE) AS "Movements in FY",
     ...
     (SELECT CASE WHEN Round((SUM(DEPOSIT_AMT_INR) - SUM(WITHDRAWAL_AMT_INR) + (SELECT BALANCE_INR - NVL(DEPOSIT_AMT_INR, 0) + NVL(WITHDRAWAL_AMT_INR, 0) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_CITI WHERE global_rank = 1)
      - (SELECT BALANCE_INR FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_CITI WHERE global_rank = (SELECT MAX(global_rank) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_CITI))),0) = 0 THEN 'Ok'
     ELSE 'Check' END FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_CITI) AS "Validation"
...
SELECT 
    ...
    (SELECT COUNT(*) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_HDFC WHERE TO_DATE(SPLIT_PART(TRANSACTION_POSTED_DATE, ' ', 1), 'DD/MM/YYYY') BETWEEN TO_DATE('01/04/2025', 'DD/MM/YYYY') AND CURRENT_DATE) AS "Movements in FY",
    TO_VARCHAR(MAX(TO_DATE(lot_dt, 'DD-MON-YY')),'DD/MM/YYYY') AS "Last Data Load Date",
    ...
    (SELECT CASE WHEN Round((SUM(DEPOSIT_AMT_INR) - SUM(WITHDRAWAL_AMT_INR) + (SELECT BALANCE_INR - NVL(DEPOSIT_AMT_INR, 0) + NVL(WITHDRAWAL_AMT_INR, 0) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_HDFC WHERE global_rank = 1)
      - (SELECT BALANCE_INR FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_HDFC WHERE global_rank = (SELECT MAX(global_rank) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_HDFC))),0) = 0 THEN 'Ok'
    ELSE 'Check' END FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_HDFC) AS "Validation"
...
SELECT 
    ...
    'ICICI Bank' AS "Bank",
     ...
     (SELECT COUNT(*) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_ICICI WHERE TO_DATE(SPLIT_PART(TRANSACTION_POSTED_DATE, ' ', 1), 'DD/MM/YYYY') BETWEEN TO_DATE('01/04/2025', 'DD/MM/YYYY') AND CURRENT_DATE) AS "Movements in FY",
     TO_VARCHAR(MAX(TO_DATE(lot_dt, 'DD-MON-YY')),'DD/MM/YYYY') AS "Last Data Load Date",
     ...
     (SELECT CASE WHEN Round((SUM(DEPOSIT_AMT_INR) - SUM(WITHDRAWAL_AMT_INR) + (SELECT BALANCE_INR - NVL(DEPOSIT_AMT_INR, 0) + NVL(WITHDRAWAL_AMT_INR, 0) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_ICICI WHERE global_rank = 1)
      - (SELECT BALANCE_INR FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_ICICI WHERE global_rank = (SELECT MAX(global_rank) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_ICICI))),0) = 0 THEN 'Ok'
    ELSE 'Check' END FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_ICICI) AS "Validation"
...
SELECT 
    ...
    (SELECT COUNT(*) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_SARAS WHERE TO_DATE(SPLIT_PART(TRANSACTION_POSTED_DATE, ' ', 1), 'DD/MM/YYYY') BETWEEN TO_DATE('01/04/2025', 'DD/MM/YYYY') AND CURRENT_DATE) AS "Movements in FY",
    TO_VARCHAR(MAX(TO_DATE(lot_dt, 'DD-MON-YY')),'DD/MM/YYYY') AS "Last Data Load Date",
    ...
    (SELECT CASE WHEN Round((SUM(DEPOSIT_AMT_INR) - SUM(WITHDRAWAL_AMT_INR) + (SELECT BALANCE_INR - NVL(DEPOSIT_AMT_INR, 0) + NVL(WITHDRAWAL_AMT_INR, 0) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_SARAS WHERE global_rank = 1)
    - (SELECT BALANCE_INR FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_SARAS WHERE global_rank = (SELECT MAX(global_rank) FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_SARAS))),0) = 0 THEN 'Ok'
    ELSE 'Check' END FROM GROUP_SHARE_INDIA.FINANCE.MARTS_CASH_FLOW_SARAS) AS "Validation"
