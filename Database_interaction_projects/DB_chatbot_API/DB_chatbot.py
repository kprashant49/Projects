...
def build_sql_from_input(user_input: str) -> str:
    """Generate SQL from natural language input using LLM."""
    prompt = f"""
        You are a MySQL query generator.
        - Users will provide keywords or natural language (they may NOT know the exact column names).
        - Always generate a valid SQL SELECT query for MySQL.
...
def query_builder():
    """CLI version: asks user for input, reuses build_sql_from_input."""
    user_input = input("Please enter what do you wish to query? ")
    return build_sql_from_input(user_input)
