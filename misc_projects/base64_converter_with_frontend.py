...
def detect_file_type(encoded_data: str):
...
if uploaded_file and mode == "Encode":
    st.success("File ready for encoding")

    raw_bytes = uploaded_file.read()
    encoded = base64.b64encode(raw_bytes).decode("utf-8")

    st.text_area("Base64 Output", encoded, height=200)
    ...
    st.success("Base64 file ready for decoding")
    ...
    st.info(f"Detected file type: {detected_ext}")

    st.download_button(
        label="Download Decoded File",
        data=decoded_bytes,
        file_name=f"decoded{detected_ext}",
